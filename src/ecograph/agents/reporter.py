...
def _build_markdown_report(
    ...
    for sid, data in list(sat_data.items())[:15]:
        ...
        flux = data.get("flux_tco2yr", "N/A")
        flux_fmt = f"{flux:,.0f}" if isinstance(flux, (int, float)) else str(flux)
        sat_rows += (
            f"| {sid} | {data.get('reported_tco2yr', 'N/A')} | "
            f"{flux_fmt} | {data.get('method', 'N/A')} | {flag} |\n"
        )
    ...
    hotspots_list = "\n".join(f"- {sid}" for sid in hotspots) if hotspots else "- None identified"
    violations_text = ""
    if violations:
        violations_text = "**Violations:**\n" + "\n".join(f"- {v}" for v in violations)

    errors_text = "No errors reported." if not errors else "\n".join(f"- {e}" for e in errors[:10])
...
| :--- | :--- | :--- | :--- | :--- |
{sat_rows or "| N/A | N/A | N/A | N/A | N/A |"}

---

## 5. Mitigation Plan

**Total projected reduction:** {total_reduction:,.0f} tCO2e/year ({total_reduction/max(baseline, 1)*100:.1f}% of baseline)

| Rank | Action Type | CO2 Reduction (tCO2/yr) | Timeline (months) | Feasibility | Score |
| :--- | :--- | :--- | :--- | :--- | :--- |
{rec_rows or "| N/A | N/A | N/A | N/A | N/A | N/A |"}
