...
from ecograph.llm import ILLMClient, LLMQuotaExhaustedError, get_groq_client  # Fix: was LLMClient
...
def _find_alternative_suppliers(hotspot_ids: list[str]) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Alternative supplier lookup failed: %s", exc)
        return []
...
def run(state: EcoState) -> EcoState:
    ...
    try:
        ...
        logger.warning("SupplyCommander: could not parse structured plan: %s", exc)
