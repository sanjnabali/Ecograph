...
from app.theme import apply_theme, section_header, empty_state, badge  # Fix: 'badfe' -> 'badge'
...
def _load(limit):
    try:
        ...
        rows = db.execute_read(
            ...
            "OPTIONAL MATCH (s)-[:HAS_OBSERVATION]->(obs:Observation) WHERE obs.metric = 'co2_flux_tonnes_per_year' "
            "WITH s, avg(obs.value) AS sat_co2 "
            "RETURN s.entity_id AS name, s.country AS country, s.co2_scope3 AS reported_co2, "
            "sat_co2 AS satellite_co2, s.discrepancy_flag AS discrepancy_flag "
            "ORDER BY s.co2_scope3 DESC LIMIT $lim",
            {"lim": limit},
        )
        ...
        def _disc(row):
            ...
            if pd.isna(r) or pd.isna(s) or max(r, s) == 0:
                return None
        ...
        def _status(row):
            if pd.isna(row["satellite_co2"]):
                return "No Data"
            if (row.get("discrepancy_pct") or 0) > 20:
...
if flag_only:
    df = df[df["status"] == "Discrepancy flagged"]
...
with k4:
    change_class = "negative" if discrepancy_count > 0 else "neutral"
    change_text = "⚠️ Requires review" if discrepancy_count > 0 else "✓ All clear"
    st.markdown(
        ...
        f'<div class="kpi-change {change_class}">{change_text}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
color_map = {"Verified": "#22C55E", "No Data": "#94A3B8", "Discrepancy flagged": "#EF4444"}
chart_df = df.sort_values("reported_co2", ascending=True).tail(top_n)

fig = px.bar(
    chart_df, x="reported_co2", y="name", orientation="h", color="status",
...
try:
    ...
    scope_rows = db.execute_read(
        "MATCH (s:Supplier)-[:HAS_OBSERVATION]->(obs:Observation) "
        "WHERE obs.metric IN ['co2_scope1', 'co2_scope2', 'co2_scope3'] "
        "RETURN coalesce(s.name, s.entity_id) AS name, obs.metric AS metric, obs.value AS value "
        "ORDER BY name",
        {},
    )
    if scope_rows:
